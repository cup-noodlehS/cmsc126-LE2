...
export function BudgetProgressBar({ categoryBudgets, categories }: BudgetProgressBarProps) {
        ...
        {segments.map((seg, idx) => (
          <div
            ...
            style={{
              ...
              borderTopRightRadius: idx === segments.length - 1 ? '0.75rem' : 0,
              borderBottomRightRadius: idx === segments.length - 1 ? '0.75rem' : 0,
            }}
            title={`${seg.name}: ${seg.amount.toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })}`}
          />
        ))}
      </div>
      <hr className="my-3 border-gray-300 dark:border-gray-700" />
      <div className="flex flex-wrap gap-2 mt-2 text-xs text-gray-700 dark:text-gray-300">
        {segments.map(seg => (
          <span key={seg.key} className="flex items-center gap-1">
            ...
            {seg.name} ({seg.amount.toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })})
          </span>
        ))}
      </div>
    </div>
